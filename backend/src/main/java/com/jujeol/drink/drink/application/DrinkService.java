...
//        }
        ...
        List<DrinkDto> drinkDtos = drinkRepository.findAllSortByPreference(Pageable.ofSize(8))
                .getContent()
                .stream()
                .map(drink -> DrinkDto.create(drink, Preference.anonymousPreference(drink), 0))
                .collect(Collectors.toList());
