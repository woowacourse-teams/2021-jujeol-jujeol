...
export const renderDrinksTable = async (drinks) => {
  ...
  const response = await getRequest(`/admin/drinks?page=${currentPage ?? 1}`, true);
  ...
  drinks.getDrinks().forEach((element) => {
    $drinksTable.querySelector('tbody').insertAdjacentHTML('beforeend', drinkTableRow(element));
  });
};
...
export const renderDrinksBatchInsertModal = async () => {
  ...
  for (let i = 0; i < BATCH_SIZE; i++) {
    $drinksTable.querySelector('tbody').insertAdjacentHTML('afterbegin', drinkInsertModalRow(result.data));
  }
};
...
function convertRequestToDrinkArray(drinkResponse) {
  return drinkResponse.map(
    (ele) =>
      new Drink(ele.id, ele.name, ele.englishName, ele.alcoholByVolume, ele.imageUrl, ele.description, ele.category)
  );
}
...
function renderDrinkPaginationButton(currentPage, lastPage) {
  ...
  for (let i = start; i <= last; i++) {
    $drinkPagination.insertAdjacentHTML('beforeend', i === currentPage ? pageItemSelected(i) : pageItem(i, i));
  }
